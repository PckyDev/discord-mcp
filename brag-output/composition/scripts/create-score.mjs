// Original deterministic 110 BPM electronic score. No sampled music or voices.
import { mkdirSync, writeFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
const out = fileURLToPath(new URL('../assets/', import.meta.url));
mkdirSync(out, { recursive: true });
const sr = 44100, seconds = 25, count = sr * seconds, beat = 60 / 110;
...
for (let bar = 0; bar < 11; bar++) {
  ...
  for (let b = 0; b < 8; b++) {
    const t = start + b * beat / 2;
    if (t > 22) break;
    note(t, 1.35, chord[[0,2,1,3,2,1,3,2][b]] + 24, .15, b % 2 ? .65 : -.65);
  }
  if (bar > 0 && start < 21.4) for (let b = 0; b < 4; b++) {
...
[50,57,61,66,74].forEach((m, i) => note(21.273 + i * .04, 3.7 - i * .04, m, .10, (i - 2) * .2, 'pad'));
[78,81,86].forEach((m, i) => note(21.273 + i * .12, 2.8, m, .11, (i - 1) * .5));
...
for (let i = 0; i < count; i++) {
  const t = i / sr, fade = Math.min(1, t / .1, Math.max(0, (25 - t) / 1.5));
...
console.log('Original score: 25 seconds, 110 BPM, stereo 44.1 kHz.');
